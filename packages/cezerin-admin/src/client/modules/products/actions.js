...
export function deleteProducts() {
  return (dispatch, getState) => {
    const state = getState();
    const promises = state.products.selected.map(productId => api.products.delete(productId));
...
export function setCategory(category_id) {
  return (dispatch, getState) => {
    const state = getState();
    const promises = state.products.selected.map(productId => api.products.update(productId, { category_id }));
...
export function updateImages(productId, images) {
  return (dispatch, getState) => {
    const promises = images.map(image => api.products.images.update(productId, image.id, image));
