...
const ProductShort = ({ id, name, thumbnailUrl, priceFormatted, enabled, discontinued, actions }) => (
  <div
    className={
      style.relatedProduct + (enabled === false || discontinued === true ? ` ${style.relatedProductDisabled}` : '')
    }
  >
    <div className={style.relatedProductImage}>{thumbnailUrl && thumbnailUrl !== '' && <img src={thumbnailUrl} />}</div>
...
const RelatedProductActions = ({ fields, index }) => (
  ...
  >
    ...
    {index > 0 && <MenuItem primaryText={messages.actions_moveUp} onClick={() => fields.move(index, index - 1)} />}
...
const RelatedProduct = ({ settings, product, actions }) => {
  ...
  return <ProductShort id="-" name="" thumbnailUrl="" priceFormatted="" actions={actions} />;
};
  ...
  <form onSubmit={handleSubmit}>
    <Paper className="paper-box" zDepth={1}>
      <div className={style.innerBox}>
        ...
        >
          <div className="col-xs-12 col-sm-4">{messages.additionalCategories}</div>
          <div className="col-xs-12 col-sm-8">
