...
export default class ProductImages extends React.Component {
  ...
  render() {
    ...
    const dialogButtons = [
      <FlatButton label={messages.cancel} onClick={this.closeEdit} style={{ marginRight: 10 }} />,
      <FlatButton label={messages.save} primary keyboardFocused onClick={this.handleEditSave} />
    ];
    ...
    return (
      <Paper className="paper-box" zDepth={1}>
        <div style={{ padding: '10px 10px 30px 10px' }}>
          ...
          >
            <TextField floatingLabelText={messages.alt} fullWidth value={alt} onChange={this.handleAltChange} />
          </Dialog>
        </div>
      </Paper>
    );
