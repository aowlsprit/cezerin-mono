...
const RedirectItem = ({ redirect }) => (
  <div>
    <Divider />
    <Link to={`/admin/settings/redirects/${redirect.id}`} style={{ textDecoration: 'none' }}>
      <ListItem
...
export default class RedirectsList extends React.Component {
  ...
  render() {
    ...
    const listItems = redirects.map((redirect, index) => <RedirectItem key={index} redirect={redirect} />);

    return (
      <div>
        <div style={{ margin: 20, color: 'rgba(0, 0, 0, 0.52)' }}>{messages.redirectsAbout}</div>
