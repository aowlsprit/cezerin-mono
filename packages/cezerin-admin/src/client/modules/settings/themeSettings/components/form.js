...
class ThemeSettings extends React.Component {
  ...
  render() {
    const { handleSubmit, pristine, submitting, initialValues, reset, settingsSchema } = this.props;
    if (initialValues && settingsSchema) {
      ...
      const fields = sortedSettingsSchema.map((item, index) => {
        ...
        if (item.section !== lastSection) {
          ...
          sectionTitle =
            item.section && item.section !== '' ? <div className={style.sectionTitle}>{item.section}</div> : null;
        }
