...
const DynamicEditControl = ({ type, fieldName, label, options, properties }) => {
  ...
  if (type === 'string' && !hasOptions) {
    return <Field component={TextField} fullWidth name={fieldName} floatingLabelText={label} />;
  }
  if (type === 'string' && hasOptions) {
    ...
    return (
      <Field component={SelectField} name={fieldName} floatingLabelText={label} fullWidth autoWidth>
        {selectOptions}
      </Field>
    );
  }
  ...
  if (type === 'boolean') {
    return (
      <Field component={CustomToggle} name={fieldName} label={label} style={{ paddingTop: 16, paddingBottom: 16 }} />
    );
  }
