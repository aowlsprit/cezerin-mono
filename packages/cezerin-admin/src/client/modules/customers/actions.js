...
export function deleteCustomers() {
  return (dispatch, getState) => {
    const state = getState();
    const promises = state.customers.selected.map(customerId => api.customers.delete(customerId));
...
export function setGroup(group_id) {
  return (dispatch, getState) => {
    const state = getState();
    const promises = state.customers.selected.map(customerId => api.customers.update(customerId, { group_id }));
