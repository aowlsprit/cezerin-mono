...
export function bulkUpdate(dataToSet) {
  return (dispatch, getState) => {
    ...
    const state = getState();
    const promises = state.orders.selected.map(orderId => api.orders.update(orderId, dataToSet));
...
export function deleteOrders() {
  return (dispatch, getState) => {
    const state = getState();
    const promises = state.orders.selected.map(orderId => api.orders.delete(orderId));
