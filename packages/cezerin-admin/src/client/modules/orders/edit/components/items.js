...
const ProductOptions = ({ options, onChange, selectedOptions }) => {
  if (options) {
    const items = options.map((option, index) => (
      <ProductOption key={index} option={option} onChange={onChange} selectedOptions={selectedOptions} />
    ));
...
export class OrderItem extends React.Component {
  ...
  render() {
    ...
    const editFormActions = [
      <FlatButton label={messages.cancel} onClick={this.hideEditForm} style={{ marginRight: 10 }} />,
      ...
      <div>
        <div className={`${style.item} row row--no-gutter middle-xs`}>
          <div className="col-xs-2">
            {thumbnailUrl && thumbnailUrl !== '' && <img src={thumbnailUrl} className={style.itemImage} />}
          </div>
          ...
          <div className="col-xs-2" style={{ textAlign: 'right' }}>
            {priceTotal}
            {item.discount_total > 0 && <small className={style.itemDiscount}>{discountTotal}</small>}
          </div>
          <div className="col-xs-1" style={{ textAlign: 'center' }}>
            {allowEdit && (
              <IconMenu
