...
export default class ServicesList extends React.Component {
  ...
  render() {
    ...
    if (services && services.data) {
      serviceItems = services.data.map((service, index) => <ServiceItem key={index} service={service} />);
    }

    const appItems = apps.map((app, index) => <AppItem key={index} app={app.Description} />);
