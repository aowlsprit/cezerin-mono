...
const DrawerMenu = ({ open, onClose, currentUrl }) => {
  const items = menuItems.map((item, index) =>
    item.title === '-' ? (
      ...
      <NavLink to={item.url} key={index} exact style={styles.link} activeStyle={styles.linkActive}>
        <MenuItem
          onClick={onClose}
          ...
          style={styles.item}
          leftIcon={
            <FontIcon style={item.url === currentUrl ? styles.iconActive : styles.icon} className="material-icons">
              {item.icon}
            </FontIcon>
          }
        />
