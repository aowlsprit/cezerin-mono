...
const sitemapRendering = (req, res) => {
  Promise.all([api.sitemap.list({ enabled: true }), api.settings.retrieve()]).then(
    ([sitemapResponse, settingsResponse]) => {
      const sitemapArray = sitemapResponse.json;
      const settings = settingsResponse.json;
      const hostname =
        settings.domain && settings.domain.length > 0 ? settings.domain : `${req.protocol}://${req.hostname}`;

      const urls = sitemapArray
        .filter(item => item.type !== 'reserved' && item.type !== 'search' && !SITEMAP_EXCLUDE_PATH.includes(item.path))
        .map(item => item.path);
