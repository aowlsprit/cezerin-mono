  ...
  return (
    <div className="columns is-mobile">
      ...
      <div className="column">
        ...
        {item.variant_name.length > 0 && <div className="cart-option-name">{item.variant_name}</div>}
        <div className="qty">
          <span>{text.qty}:</span>
...
const OrderSummary = props => {
  ...
  if (cart && cart.items && cart.items.length > 0) {
    const items = cart.items.map(item => (
      <SummaryItem key={item.id} item={item} updateCartItemQuantiry={updateCartItemQuantiry} settings={settings} />
    ));
    ...
    return (
      <div className="checkout-box content is-small" style={{ paddingBottom: 0 }}>
        ...
        <div className="columns is-mobile is-gapless is-multiline summary-block">
          ...
          <div className="column is-5 has-text-right price">{helper.formatCurrency(cart.subtotal, settings)}</div>
          <div className="column is-7">{text.shipping}</div>
          <div className="column is-5 has-text-right price">{helper.formatCurrency(cart.shipping_total, settings)}</div>
