...
const LoadMore = ({ loadMoreProducts, hasMore, loading, className = 'button is-fullwidth is-dark' }) => {
  if (hasMore) {
    const buttonStyle = {};
    ...
    const loadMoreText =
      themeSettings.button_loadmore_text && themeSettings.button_loadmore_text.length > 0
        ? themeSettings.button_loadmore_text
        : text.loadMore;

    return (
      <button onClick={loadMoreProducts} className={className + (loading ? ' is-loading' : '')} style={buttonStyle}>
        {loadMoreText}
      </button>
    );
  }
