    ...
    return (
      <div className="price-filter">
        ...
        <div className="columns is-mobile is-gapless price-filter-values">
          <div className="column has-text-left">{helper.formatCurrency(this.state.minValue, settings)}</div>
          <div className="column has-text-right">{helper.formatCurrency(this.state.maxValue, settings)}</div>
        </div>
      </div>
    );
  }
