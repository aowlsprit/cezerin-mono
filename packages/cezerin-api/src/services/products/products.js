...
class ProductsService {
  ...
  getOrganizedAttributes(allAttributesResult, filteredAttributesResult, params) {
    ...
    return uniqueAttributesName.sort().map(attributeName => ({
      ...
      values: allAttributesResult
        ...
        .map(b => ({
          name: b._id.value,
          checked: !!(params[`attributes.${b._id.name}`] && params[`attributes.${b._id.name}`].includes(b._id.value)),
          // total: b.count,
  ...
  getMatchQuery(params, categories, useAttributes = true, usePrice = true) {
    let { category_id, enabled, discontinued, on_sale, stock_status, price_from, price_to, sku, ids, tags } = params;
  ...
  getSingleProduct(id) {
    ...
    return this.getProducts({ ids: id, limit: 1 }).then(products => (products.data.length > 0 ? products.data[0] : {}));
  }

  addProduct(data) {
    return this.getValidDocumentForInsert(data)
      .then(dataToInsert => db.collection('products').insertMany([dataToInsert]))
  ...
  updateProduct(id, data) {
    ...
    return this.getValidDocumentForUpdate(id, data)
      .then(dataToSet => db.collection('products').updateOne({ _id: productObjectID }, { $set: dataToSet }))
    ...
    return db
      ...
      .then(deleteResponse => {
        if (deleteResponse.deletedCount > 0) {
          ...
          const deleteDir = path.resolve(`${settings.productsUploadPath}/${productId}`);
