...
class ProductCategoriesService {
  ...
  getSingleCategory(id) {
    ...
    return this.getCategories({ id }).then(categories => (categories.length > 0 ? categories[0] : null));
  }
    ...
    return this.getCategories()
      ...
      .then(idsToDelete =>
        ...
        idsToDelete
          ? db
              .collection('products')
              .updateMany({ category_ids: { $all: idsToDelete } }, { $pull: { category_ids: { $all: idsToDelete } } })
              .then(() => idsToDelete)
          : null
      )
      .then(idsToDelete => {
        ...
        if (idsToDelete) {
          for (const categoryId of idsToDelete) {
            const deleteDir = path.resolve(`${settings.categoriesUploadPath}/${categoryId}`);
  ...
  uploadCategoryImage(req, res) {
    ...
    form
      .on('fileBegin', (name, file) => {
        ...
        const dir = path.resolve(`${settings.categoriesUploadPath}/${categoryId}`);
        fse.emptyDirSync(dir);
