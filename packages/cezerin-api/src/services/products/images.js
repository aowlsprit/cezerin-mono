    ...
    return this.getImages(productId)
      .then(images => {
        if (images && images.length > 0) {
          const imageData = images.find(i => i.id.toString() === imageId.toString());
          if (imageData) {
            const { filename } = imageData;
            const filepath = path.resolve(`${settings.productsUploadPath}/${productId}/${filename}`);
            fse.removeSync(filepath);
            return db
              .collection('products')
              .updateOne({ _id: productObjectID }, { $pull: { images: { id: imageObjectID } } });
          }
          return true;
        }
        return true;
