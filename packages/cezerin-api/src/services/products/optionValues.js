...
class ProductOptionValuesService {
  ...
  getOptionValues(productId, optionId) {
    ...
    return db
      ...
      .then(options =>
        options && options.length > 0 ? options.find(option => option.id.toString() === optionId) : null
      )
  ...
  overwriteAllValuesForOption(productId, optionId, values) {
    ...
    return db
      .collection('products')
      .updateOne({ _id: productObjectID, 'options.id': optionObjectID }, { $set: { 'options.$.values': values } });
  }
