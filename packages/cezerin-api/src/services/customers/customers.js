...
class CustomersService {
  ...
  getSingleCustomer(id) {
    ...
    return this.getCustomers({ id }).then(items => (items.data.length > 0 ? items.data[0] : {}));
  }
  ...
  async addCustomer(data) {
    ...
    if (customer.email && customer.email.length > 0) {
      const customerCount = await db.collection('customers').count({ email: customer.email });
      if (customerCount > 0) {
  ...
  validateAddresses(addresses) {
    if (addresses && addresses.length > 0) {
      const validAddresses = addresses.map(addressItem => parse.getCustomerAddress(addressItem));
  ...
  changeProperties(customer, customerGroups) {
    if (customer) {
      ...
      if (customer.addresses && customer.addresses.length === 1) {
        ...
        const default_billing = customer.addresses.find(address => address.default_billing);
        const default_shipping = customer.addresses.find(address => address.default_shipping);
