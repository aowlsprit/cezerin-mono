...
class PaymentMethodsService {
  ...
  getMethods(params = {}) {
    return this.getFilter(params).then(filter => PaymentMethodsLightService.getMethods(filter));
  }
  ...
  getSingleMethod(id) {
    ...
    return this.getMethods({ id }).then(methods => (methods.length > 0 ? methods[0] : null));
  }
  ...
  async pullShippingMethod(id) {
    ...
    return db
      .collection('paymentMethods')
      .update({}, { $pull: { 'conditions.shipping_method_ids': methodObjectID } }, { multi: true });
  }

  getPaymentMethodConditions(conditions) {
    const methodIds = conditions ? parse.getArrayIfValid(conditions.shipping_method_ids) || [] : [];
    let methodObjects = [];
