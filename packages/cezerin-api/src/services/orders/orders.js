...
class OrdersService {
      ...
      if (!order.customer_id && order.email) {
        ...
        return CustomersService.getCustomers({ email: order.email }).then(customers => {
          ...
          const customerrFullName =
            order.shipping_address && order.shipping_address.full_name ? order.shipping_address.full_name : '';
  ...
  async updateOrder(id, data) {
    ...
    const updateResponse = await db.collection('orders').updateOne({ _id: orderObjectID }, { $set: orderData });
  ...
  changeProperties(order, orderStatuses, shippingMethods, paymentMethods) {
    if (order) {
      ...
      const tax_included_total =
        (order.item_tax_included ? 0 : order.item_tax) + (order.shipping_tax_included ? 0 : order.shipping_tax);
      ...
      const grand_total = sum_items_price_total + shipping_total + tax_included_total - discount_total;
