...
class SecurityTokensService {
  ...
  getSingleToken(id) {
    ...
    return this.getTokens({ id }).then(items => (items.length > 0 ? items[0] : null));
  }

  getSingleTokenByEmail(email) {
    return this.getTokens({ email }).then(items => (items.length > 0 ? items[0] : null));
  }
  ...
  checkTokenEmailUnique(email) {
    if (email && email.length > 0) {
      return db
        ...
        .then(count => (count === 0 ? email : Promise.reject('Token email must be unique')));
    }
      ...
      this.getSingleTokenByEmail(email).then(token => {
        if (token) {
          return this.getSignedToken(token).then(signedToken => {
            const loginUrl = url.resolve(settings.adminBaseURL, settings.adminLoginPath);
